...
def model_grounding_payload(payload: dict[str, Any]) -> dict[str, Any]:
    entities = payload["entities"]
    relationships = payload["relationships"]
    health_state_counts: dict[str, int] = {}
    for entity in entities:
        state = entity.get("healthState") or "Unknown"
        health_state_counts[state] = health_state_counts.get(state, 0) + 1
    return {
        "observedAt": payload["observedAt"],
        "model": payload["model"],
        "entityCount": len(entities),
        "relationshipCount": len(relationships),
        "healthStateCounts": health_state_counts,
        "entities": [
            {
                "name": entity.get("name"),
                "displayName": entity.get("displayName"),
                "healthState": entity.get("healthState") or "Unknown",
                "signals": [
                    {
                        "name": signal.get("name"),
                        "healthState": signal.get("healthState") or "Unknown",
                        "value": signal.get("value"),
                        "reportedAt": signal.get("reportedAt"),
                    }
                    for signal in entity.get("signals", [])
                ],
            }
            for entity in entities
        ],
