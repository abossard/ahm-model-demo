...
class HealthApiClientTests(unittest.TestCase):
    ...
    def test_reads_fresh_rich_model_and_exact_entity_boundaries(self):
        ...
        grounded_model = model_grounding_payload(actual_inventory)
        self.assertEqual(grounded_model["entityCount"], 2)
        self.assertEqual(grounded_model["relationshipCount"], 1)
        self.assertEqual(
            grounded_model["healthStateCounts"],
            {"Degraded": 1, "Healthy": 1},
        )
        self.assertEqual(
            grounded_model["entities"],
            [
                {
                    "name": "api",
                    "displayName": None,
    ...
    def test_noncustom_approval_reason_matches_exact_post_payload(self):
        cases = [
            ("maintenance", "Model supplied conflicting maintenance detail"),
            ("recovery", "Model supplied conflicting recovery detail"),
        ]
        for index, (preset, visible_reason) in enumerate(cases):
            with self.subTest(preset=preset):
                ...
                self.assertEqual(
                    request.url.path, "/api/entities/api/health-reports"
                )
                self.assertEqual(
