...
app = Flask(__name__)
app.config["MAX_CONTENT_LENGTH"] = AGENT_MAX_BODY_BYTES
...
def _copilot_enabled(environment):
...
def _connection_tokens(headers):
    result = set()
    values = (
        headers.getlist("Connection")
        if hasattr(headers, "getlist")
        else headers.get_list("Connection")
    )
    for value in values:
        result.update(
...
def _agent_response_headers(upstream_headers):
    blocked = HOP_BY_HOP_HEADERS | _connection_tokens(upstream_headers)
    result = []
...
def _agent_path_kind(agent_path):
...
def _agent_request_is_unambiguous():
    raw_uri = (
...
def _agent_body_chunks():
    remaining = request.content_length or 0
    while remaining:
        chunk = request.stream.read(min(remaining, AGENT_BODY_CHUNK_BYTES))
        if not chunk:
            break
        remaining -= len(chunk)
        yield chunk
...
def _agent_error(status, code, retryable):
...
def apply_response_headers(response):
    if request.path == "/agent" or request.path.startswith("/agent/"):
        response.headers.setdefault(
            "Content-Security-Policy",
            AGENT_ERROR_SECURITY_POLICY,
        )
    else:
        response.headers["Content-Security-Policy"] = PARENT_SECURITY_POLICY
...
def _proxy_agent(agent_path):
    if not _copilot_enabled(os.environ):
        return _error_response(
            404,
            "agent_disabled",
    ...
    if not _agent_request_is_unambiguous():
        return _validation_error(
            "invalid_agent_path",
            "The agent path is not supported.",
        )

    path_kind = _agent_path_kind(agent_path)
    ...
    allowed_methods = (
        {"GET", "HEAD"}
        if path_kind in {"document", "asset"}
    ...
    if has_body and request.mimetype != "application/json":
        return _error_response(
            415,
    ...
    if not _agent_proxy_slots.acquire(blocking=False):
        return _agent_error(503, "agent_proxy_saturated", True)

    suffix = f"/{agent_path}" if agent_path else ""
    upstream_url = f"{AGENT_WEB_ORIGIN}/agent{suffix}"
    if request.query_string:
        upstream_url += f"?{request.query_string.decode('ascii')}"
    ...
    try:
        upstream_request = client.build_request(
            ...
            content=_agent_body_chunks() if has_body else None,
        )
        ...
        client.close()
        _agent_proxy_slots.release()
        return _agent_error(502, "agent_web_unavailable", True)
    except Exception as error:
    ...
    headers = _agent_response_headers(upstream.headers)
    if 300 <= upstream.status_code < 400:
        location = _safe_agent_location(
            upstream_url,
        ...
        if location is None:
            ...
            client.close()
            _agent_proxy_slots.release()
            return _agent_error(502, "agent_redirect_rejected", False)
    ...
    if request.method == "HEAD":
        ...
        client.close()
        _agent_proxy_slots.release()
        response = Response(status=upstream.status_code, headers=headers)
...
def agent_path(agent_path):
    return _proxy_agent(agent_path)


@app.get("/")
def index():
    response_formats = request.args.getlist("format")
    if not response_formats:
        return render_template(
            "index.html",
            copilot_enabled=_copilot_enabled(os.environ),
        )
