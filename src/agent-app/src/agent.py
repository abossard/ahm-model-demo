...
def create_agent(
    ...
    def read_health_model() -> str:
        return json.dumps(
            model_grounding_payload(health_api.read_health_model()),
            separators=(",", ":"),
        )
    ...
    base_agent = Agent(
        ...
        instructions=dedent(
            ...
            - For every model-wide question, call read_health_model during the
              current turn. Copy entityCount, relationshipCount, observedAt,
              and healthStateCounts exactly from the tool result; never count
              arrays yourself. Include exact entity, relationship, state, and
              signal facts relevant to the answer.
